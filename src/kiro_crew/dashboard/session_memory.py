            ...
            sessions_out.append(
                {
                    ...
                    "cpu_cores": (sample or {}).get("cpu_cores"),
                    "uptime_s": (
                        round(now_wall - created, 1) if isinstance(created, float) else None
                    ),
                }
            )
            # Count each runtime ONCE. Co-tenants report the same pid, so adding
            # every row would multiply a shared runtime into the total.
