...
async def _live_worktree_path(*, fresh: bool = False) -> str | None:
    ...
    _LIVE_CHECK_AT = now
    # The live-target pointer outranks every service-definition probe below: a
    # cutover always writes the pointer, and on a host whose service cannot be
    # driven it writes ONLY the pointer — so the unit's WorkingDirectory still
    # names the checkout the gateway was installed from. Reading the definition
    # first would report that stale checkout as live and leave `already_live`
    # and `is_live` permanently wrong.
    ...
    async with _MAKE_LIVE_LOCK:
        ...
        if not can_restart:
            ...
            return {"ok": True, "cutover": True, "staged_only": True,
                    ...
                    "notice": (
                        f"{real.name} is now the live target, but this gateway is "
                        f"not a service Dev Fleet can restart "
                        f"({_make_live_status_error(unit_status)}). Run "
                        f"`{_manual_restart_command()}` to finish the cutover — "
                        "the gateway will come up on the new target."
                    )}
