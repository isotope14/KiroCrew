...
def snapshot() -> str | None:
    ...
    Only absence maps to ``None``; any other :class:`OSError` propagates. The
    caller uses this to make a cutover reversible, and ``restore(None)`` DELETES
    the pointer — so reporting an unreadable file as "there was nothing here"
    would let a failed cutover destroy a live target it merely could not read.
    """
    try:
        return pointer_path().read_text()
    except FileNotFoundError:
        return None
